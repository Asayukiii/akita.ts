...
export const data: SourceFunction = {
    data: new FunctionBuilder()
        .setName('addRoleMenu')
        .setValue('description', 'add a role select menu component at the instance.')
        .setValue('use', '$addRoleMenu[placeholder?;customId;disabled?;minValues?;maxValues?]')
        .setValue('fields', [{
            name: 'placeholder',
        ...
        .setValue('example', '$addRoleMenu[select some channel;channel_menu_1;text]')
        .setValue('returns', 'Void'),
    code: async function () {
        await this.resolveFields()
        let [placeholder, customId, disabled = "false", min = 1, max = 1] = this.fields.split(true) as string[]
        this.meta.ctn.addRoleMenu({
